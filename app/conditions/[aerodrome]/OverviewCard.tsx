        ...
        <div className="flex flex-col items-center justify-center shrink-0">
          {bestRunway && metar ? (
            ...
            <div className="w-[220px] p-4">
              <div className="text-xs text-slate-400 uppercase tracking-wide mb-2">Runways</div>
              <div className="space-y-2">
                {runways.map((rwy) => (
                  <div key={rwy.id} className="bg-slate-900/30 rounded-lg p-2">
                    <div className="text-xl font-bold text-white">{rwy.id}</div>
                    <div className="text-xs text-slate-400 mt-1">
                      {rwy.length.toLocaleString()} ft × {rwy.width} ft
                    </div>
                    <div className="text-xs text-slate-500">
                      {rwy.surfaceName}
                      {rwy.lighted && <span className="text-yellow-400 ml-1">Lighted</span>}
                    </div>
                  </div>
                ))}
              </div>
